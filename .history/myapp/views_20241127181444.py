...
def menu(request):
    ...
    dishes = Dish.objects.filter(is_available=True)  # Lọc các món ăn có sẵn
    menu = [
        {
            "cat_id": category.id,
            "cat_name": category.name,
            "items": dishes.filter(category=category).values('id', 'name', 'price', 'discounted_price')
        }
    ...
    if request.method == 'POST':
        try:
            ...
            bill = Bill.objects.filter(
                table=table, customer=profile, is_payed=False).first()

            if not bill:
                return JsonResponse({"success": False, "message": "Bạn chưa đặt bàn. Vui lòng đặt bàn trước."}, status=400)
            ...
            return JsonResponse({"success": False, "message": "Không tìm thấy hồ sơ người dùng."}, status=400)

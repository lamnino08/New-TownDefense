...
def book_table(request):
    if request.method == "POST":
        ...
        # Kiểm tra nếu bàn đã có người đặt
        if table.is_occupied:
            return render(request, 'book_table.html', {
                'error_message': f"Bàn {table.name} đã được đặt.",
                'tables': Table.objects.all()
            })

        # Đặt bàn
        table.is_occupied = True
        table.current_bill = None  # Nếu cần tạo hóa đơn mới, xử lý tại đây
        table.save()

        return render(request, 'book_table.html', {
            'success_message': f"Đặt bàn {table.name} thành công!",
            'tables': Table.objects.all()
        })

    # Hiển thị tất cả các bàn
    return render(request, 'book_table.html', {
        'tables': Table.objects.all()
    })

    ...
    if request.method == "POST":
        ...
        try:
            ...
            bill.total_price = sum(
                item.dish.discounted_price * item.quantity for item in BillDish.objects.filter(bill=bill)
            )
            bill.save()
...
def my_bills(request):
    try:
        ...
        # Log khi không có hóa đơn
        if not bills.exists():
            print("Không có hóa đơn nào cho người dùng này.")

...
def view_dishes_in_table(modeladmin, request, queryset):
    for table in queryset:
        if table.is_occupied and table.current_bill:
            ...
            modeladmin.message_user(request, f"Bàn {table.name} hiện không có khách.")
    ...
    def current_bill_display(self, obj):
        ...
        if current_bill:
            ...
            return format_html(
                ...
                f'/admin/myapp/bill/add/?table={obj.id}'  # Đường dẫn tới trang tạo Bill mới
            )
    ...
    def create_order_for_table(self, request, queryset):
        for table in queryset:
            ...
            if not current_bill:
                ...
                # Thêm thông báo
                self.message_user(request, f"Order và Bill đã được tạo cho bàn {table.name}")
            else:
                self.message_user(request, f"Bàn {table.name} đã có hóa đơn chưa thanh toán.")
...
class BillAdmin(admin.ModelAdmin):
    list_display = ['id', 'table', 'customer', 'total_price', 'is_payed', 'time']
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path('<int:bill_id>/', self.admin_site.admin_view(self.bill_detail_view), name='bill_detail'),
        ]
